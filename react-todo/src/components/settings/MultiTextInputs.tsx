...
export function MultiTextInputs({ label }: { label: keyof TodoConfig }) {
  const field = useFieldContext<string[]>();
  const values = field.state.value ?? [];

  const addItem = (item: string) => {
    if (!field.state.value.includes(item)) field.pushValue(item);
  };

  const updateItem = (index: number, value: string) =>
    field.replaceValue(index, value);

  const deleteItem = (index: number) => field.removeValue(index);
